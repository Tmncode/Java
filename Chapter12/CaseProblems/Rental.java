...
public class Rental {
    ...
    public Rental(String num, int minutes, int eType)
    {
        setContractNumber(num);
        setHoursAndMinutes(minutes);
        if(eType <= Equipment.HIGH)
            equipment = new EquipmentWithLesson(eType);
        else
            equipment = new EquipmentWithoutLesson(eType);
